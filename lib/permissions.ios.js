...
const PermissionsIOS = NativeModules.R2NReactNativePermissions
...
class R2NReactNativePermissions {
  ...
  check = (permission: string, options?: CheckOptions): Promise<Status> => {
    if (!permissionTypes.includes(permission)) {
      const error = new Error(
        `R2NReactNativePermissions: ${permission} is not a valid permission type on iOS`,
      )
  ...
  request = (permission: string, options?: RequestOptions): Promise<Status> => {
    ...
    if (permission == 'backgroundRefresh') {
      const error = new Error(
        'R2NReactNativePermissions: You cannot request backgroundRefresh',
      )
...
export default new R2NReactNativePermissions()
